...
int storage_fromsql(uint32 account_id, struct storage_data* p)
{
	...
	for( i = 0; i < MAX_STORAGE && SQL_SUCCESS == Sql_NextRow(sql_handle); ++i )
	{
		...
		Sql_GetData(sql_handle,10, &data, NULL); item->rolls = atoi(data);
		
		for( j = 0; j < MAX_SLOTS; ++j ){
			Sql_GetData(sql_handle, 11+j, &data, NULL); item->card[j] = atoi(data);
		}
	}
	p->storage_amount = i;
...
int guild_storage_fromsql(int guild_id, struct guild_storage* p)
{
	...
	StringBuf_Init(&buf);
	StringBuf_AppendStr(&buf, "SELECT `id`,`nameid`,`amount`,`equip`,`identify`,`refine`,`attribute`,`bound`,`unique_id`,`rolls`");
	for( j = 0; j < MAX_SLOTS; ++j )
		StringBuf_Printf(&buf, ",`card%d`", j);
	...
	for( i = 0; i < MAX_GUILD_STORAGE && SQL_SUCCESS == Sql_NextRow(sql_handle); ++i )
	{
		...
		Sql_GetData(sql_handle, 9, &data, NULL); item->rolls = atoi(data);
		item->expire_time = 0;
		for( j = 0; j < MAX_SLOTS; ++j ){
